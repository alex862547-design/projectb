...
// ตั้ง TTL ไว้ "นานกว่า" รอบโพลของหน้าเว็บ (ตอนนี้ 10 วิ) เล็กน้อยโดยตั้งใจ — เดิมตั้งไว้สั้นกว่า (3 วิ < 4 วิ)
// ทำให้แทบทุกรอบโพลพลาดแคชและไปดึงจากฐานข้อมูลจริงซ้ำๆ โดยเปล่าประโยชน์ (กิน egress ของฐานข้อมูลฟรีๆ)
...
// "ตำแหน่งที่ตามหลัง Staff" ได้ด้วย นอกเหนือจากตำแหน่งตัวเอง — ต้องตรงกับฝั่ง frontend เป๊ะๆ
...
function checkinScopeRoles(role) {
...
async function assertCanActOnStudent(req, res, studentId) {
  ...
  if (me.role !== "หัวหน้าสี" && !checkinScopeRoles(me.role).includes(target.role)) {
...
async function assertCanConfirmTeamRole(req, res, team, role) {
  ...
  if (me.role !== "หัวหน้าสี" && !checkinScopeRoles(me.role).includes(role)) {
...
app.get("/api/attendance-messages/threads", auth(), async (req, res) => {
  ...
  if (req.user.role === "admin") {
    ...
    if (me.role === "หัวหน้าสี") {
      ...
      teamFilter = "AND s.team = $1 AND s.role = ANY($2)";
      params.push(me.team, checkinScopeRoles(me.role));
    }
  } else {
...
app.get("/api/attendance-messages/checker-unread-count", auth(), async (req, res) => {
  ...
  if (req.user.role === "student" && req.user.studentId) {
    ...
    const scopeFilter = me.role === "หัวหน้าสี" ? "" : "AND s.role = ANY($2)";
    const params = me.role === "หัวหน้าสี" ? [me.team] : [me.team, checkinScopeRoles(me.role)];
    const { rows } = await pool.query(
